...
class TimeLogsOverlapTest : TimeLogsPageTestBase() {
    ...
    fun `should show overlap warning when entries overlap by more than 1 second`() {
        ...
        testDatabaseSupport.insert(
            TimeLogEntry(
                startTime = baseTime.minusHours(2).minusMinutes(30), // 03:30 - 2:30 = 01:00
                endTime = baseTime.minusHours(1), // 03:30 - 1:00 = 02:30
                title = "Task A",
        ...
        testDatabaseSupport.insert(
            TimeLogEntry(
                startTime = baseTime.minusHours(1).minusMinutes(30), // 03:30 - 1:30 = 02:00
                endTime = baseTime.minusMinutes(30), // 03:30 - 0:30 = 03:00
                title = "Task B",
                ownerId = requireNotNull(testUser.id),
            ),
        )
    ...
    fun `should not show overlap warning when entries touch at boundary (0-1 second overlap)`() {
        ...
        testDatabaseSupport.insert(
            TimeLogEntry(
                startTime = baseTime.minusHours(2).minusMinutes(30), // 01:00
                endTime = baseTime.minusHours(1).minusMinutes(30), // 02:00
                title = "Task A",
                ownerId = requireNotNull(testUser.id),
            ),
        )
        ...
        testDatabaseSupport.insert(
            TimeLogEntry(
                startTime = baseTime.minusHours(1).minusMinutes(30), // 02:00 (same as Entry 1 end)
                endTime = baseTime.minusMinutes(30), // 03:00
                title = "Task B",
                ownerId = requireNotNull(testUser.id),
            ),
        )
    ...
    fun `should show overlap warning when one entry is fully contained in another`() {
        ...
        testDatabaseSupport.insert(
            TimeLogEntry(
                startTime = baseTime.minusHours(1).minusMinutes(30), // 02:00
                endTime = baseTime.minusMinutes(30), // 03:00
                title = "Short Task",
                ownerId = requireNotNull(testUser.id),
            ),
    ...
    fun `should show overlap warning for multiple overlapping entries`() {
        ...
        testDatabaseSupport.insert(
            TimeLogEntry(
                startTime = baseTime.minusHours(2).minusMinutes(30), // 01:00
                endTime = baseTime.minusHours(1), // 02:30
                title = "Task A",
                ownerId = requireNotNull(testUser.id),
            ),
        )
        ...
        testDatabaseSupport.insert(
            TimeLogEntry(
                startTime = baseTime.minusMinutes(30), // 03:00
                endTime = baseTime.minusSeconds(-1800), // 04:00
                title = "Task C",
                ownerId = requireNotNull(testUser.id),
            ),
    ...
    fun `should not show overlap warning for non-overlapping entries`() {
        ...
        testDatabaseSupport.insert(
            TimeLogEntry(
                startTime = baseTime.minusHours(1), // 02:30
                endTime = baseTime, // 03:30
                title = "Task B",
                ownerId = requireNotNull(testUser.id),
            ),
