...
Use this skill when the user wants to create a new Aionify release, draft a GitHub release, publish release notes, or run the local release workflow.

This skill intentionally performs all repository-writing release operations locally. CI must not create tags or GitHub releases; CI only publishes the Docker image when it sees a release version on `main`.
...
```bash
notes_file="$(mktemp -t aionify-release-notes.XXXXXX.md)"
```
...
ci_url="$(gh run list --workflow ci.yml --branch main --commit "$release_sha" --json url --jq '.[0].url' --limit 1)"
```

If no CI run is found, check whether the workflow file has a different name or whether the push has not yet been picked up.
